...
plot <- ggplot(states, mapping = aes(long, lat, group = group))  + 
  geom_polygon(fill = "light grey", colour = "dark grey") +
  coord_equal() +
  theme_void() +
  geom_point(data=locations, aes( x = as.numeric(lon), y = as.numeric(lat),fill=PID),pch=21, size=3, stroke = 0.25,inherit.aes = FALSE,position=position_jitter(h=0,w=0.12), alpha = 0.75 )+
