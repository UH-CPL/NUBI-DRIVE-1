...
Y<-Mean_Raw_HR
MMnDHR<-tmpD$Morning_Mean_Non_Drive_Hr
AMnDHR<-tmpD$Afternoon_Mean_Non_Drive_Hr
...
RelAcc<-as.factor(tmpD$Acc)
DrTraf<-as.factor(tmpD$DrTraf)
DrRain<-as.factor(tmpD$DrRain)
DrNi<-as.factor(tmpD$DrNi)
DrLike<-as.factor(tmpD$DrLike)
AGE<-tmpD$Age


TrDu<-tmpD$Trip_Duration
MSp<-tmpD$Mean_Speed
SDSp<-tmpD$SD_Speed
...
SDAccE<-tmpD$sd_acc_energy

MRotE<-tmpD$mean_rot_energy
SDRotE<-tmpD$sd_rot_energy
...
```{r, echo=FALSE}
D<-data.frame(Y,DN,Dbin,TrTi,TrNu,WI,Ge,RelAcc,DrTraf,DrRain,DrNi,DrLike,TrDu,MSp,SDSp,MATP,SDATP,MRTP,SDRTP,MJF,SDJF,
              MAccE, SDAccE, MRotE, SDRotE,
              AGE,TA,Ex,Agr,Co,Ne,Op,TNsc,SA,MMnDHR,AMnDHR,S)

VarNames<-c("Mean HR",
            "Day Number","Weekday/Weekend indicator",
            "Trip Time","Trip Number","Weather Info","Gender",
            "Relish Acceleration","Driving with Traffic","Driving with Rain",
            "Driving at Night","Like to drive",
            "Trip Duration","Mean Speed","SD Speed","Mean Absolute Throtle Position", 
            "SD Absolute Throtle Position","Mean Relative Throtle Position", 
            "SD Relative Throtle Position","Mean Jam Factor","SD Jam Factor",
            "Mean Accelerometer Energy","SD Accelerometer Energy",
            "Mean Rotation Energy","SD Rotation Energy",
            "Age","Trait Anxiety","Extraversion",
            "Agreeableness","Conscientiousness","Neuroticism","Openness",
            "Total NASA score", "State Anxiety",
            "Morning_Mean_Non_Drive_Hr","Afternoon_Mean_Non_Drive_Hr","Subject ID")
...
Non_Drive_HR <- c(MMnDHR, AMnDHR)
MAind<-c(rep("Morning", length(MMnDHR)),rep("Afternoon", length(AMnDHR)))
MAind<-factor(MAind,levels=c("Morning","Afternoon"))

WEind<-c(Dbin,Dbin)
WEind<-factor(WEind,levels=c("Weekday","Weekend"))
